...
(* Helper: Lwt-based Retry Logic (since Resilience module is now Eio-focused) *)
(* TODO: Port Resilience module to support Lwt or Functorize it *)
...
let request_json ~url ~request : (Yojson.Safe.t, api_error) result Lwt.t =
  let rec attempt n =
    ...
    match code with
    | 200 | 201 ->
        (try Lwt.return (Ok (Yojson.Safe.from_string body_str)) with Yojson.Json_error msg -> Lwt.return (Error (ParseError msg)))
    | 429 | 500 | 502 | 503 | 504 ->
        if n < figma_retry_policy.max_attempts then begin
          let delay_ms = calculate_delay figma_retry_policy n in
          let delay_sec = delay_ms /. 1000.0 in
          
          (* Add jitter *)
          let jitter = (Random.float 0.2) *. delay_sec in
          let total_delay = delay_sec +. jitter in
          
          eprintf "[Figma API] Retryable error %d, retrying in %.2fs (attempt %d/%d)\n%!" 
            code total_delay n figma_retry_policy.max_attempts;
            
          let* () = Lwt_unix.sleep total_delay in
          attempt (n + 1)
        end else
          if code = 429 then Lwt.return (Error RateLimited)
          else Lwt.return (Error (NetworkError (sprintf "HTTP %d: %s" code body_str)))
    | 401 | 403 -> Lwt.return (Error (AuthError "Invalid or expired token"))
    | 404 -> Lwt.return (Error (NotFound url))
    | _ -> Lwt.return (Error (UnknownError (code, body_str)))
  in
  attempt 1
