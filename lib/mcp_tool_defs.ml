...
let tool_figma_get_planning_context : tool_def = {
  ...
  description = "🎯 AGENT: heuristic-free planning context. 상위 에이전트가 직접 계획을 세울 수 있도록 정렬되지 않은 노드 후보, 트리 edge, preview, feature summary를 제공합니다.";
  input_schema = object_schema [
    ("file_key", string_prop "Figma 파일 키");
    ("node_id", string_prop "노드 ID (예: 123:456)");
    ("url", string_prop "Figma URL (file_key/node_id 자동 추출)");
...
let tool_figma_validate_agent_plan : tool_def = {
  ...
  description = "✅ AGENT: 상위 에이전트가 만든 plan JSON을 deterministic하게 검증합니다. node 존재 여부, dependency cycle, duplicate id, 루트 밖 참조를 확인합니다.";
  input_schema = object_schema [
    ("file_key", string_prop "Figma 파일 키");
